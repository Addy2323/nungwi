 ...
 const { t } = useLanguage()
  ...
  return <div className={styles.page}>
    ...
    <main className={styles.main}>
      <section className={styles.story} aria-label={t("Nungwi beach delivery")}><img className={styles.heroImage} src="/images/island-vibes-hero-v2.png" alt={t("Chilled island drinks on a Zanzibar beach at sunset")}/><div className={styles.storyShade}/><div className={styles.storyTop}><span><MapPin size={15}/> NUNGWI, ZANZIBAR</span><Palmtree size={35}/></div><div className={styles.storyCopy}><span>{t("MORE SUNSHINE. LESS HASSLE.")}</span><h1>{t("Your beach day,")}<br/>{t("a little ")}<em>{t("better.")}</em></h1><p>{t("Cold drinks, island favourites, and everything")}<br className={styles.desktopBreak}/>{t(" you need. Delivered to your happy place.")}</p><div className={styles.storyBenefits}><span><Truck size={19}/>{t(" Fast island delivery")}</span><span><Heart size={18}/>{t(" Your favourites, on repeat")}</span></div></div><div className={styles.storyBottom}><div className={styles.deliveryBadge}><span><ShoppingBag size={21}/></span><div><strong>{t("Stay right where you are.")}</strong><small>{t("We’ll bring the good times to you.")}</small></div></div><span className={styles.script}>{t("Same Beach")}<br/><i>{t("More Good Times")}</i></span></div></section>

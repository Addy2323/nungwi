...
import { productPath, categoryPath } from '@/lib/seo'

export default function Storefront({ shopPage = false, initialCatalogue }: { shopPage?: boolean; initialCatalogue?: Record<string, any>[] }) {
  const { t, language } = useLanguage()
  const alerts = useAlerts()
  const { products, error: catalogueError, loading } = useCatalogue(initialCatalogue)
  ...
  const notify = (message: string) => { void alerts.success(message) }
  ...
  useEffect(() => {
    ...
    if (product) setSelectedId(product)
  }, [])
  ...
  const add = (p: Product) => {
    ...
    notify(t('Added to your basket.', 'Imeongezwa kwenye kikapu chako.'))
  }
  ...
  return <div className={`storefront ${shopPage ? 'shop-page' : ''}`}>
    ...
    <main className="page-container">
      {shopPage ? <>
        <section className="shop-intro" aria-labelledby="shop-page-title">
          <nav className="shop-breadcrumb" aria-label={t('Breadcrumb', 'Njia ya ukurasa')}><Link href="/">{t('Home', 'Mwanzo')}</Link><span aria-hidden="true">/</span><span aria-current="page">{t('Shop')}</span></nav>
          <div className="shop-intro-content">
            <div><span className="eyebrow">{t('THE NUNGWI SHOP', 'DUKA LA NUNGWI')}</span><h1 id="shop-page-title">{t('Wine, spirits & island essentials.', 'Mvinyo, pombe kali na mahitaji ya kisiwani.')}</h1><p>{t('From your first cold drink to your last beach-day snack. Browse everything, find your favourites, and we will bring them to you.', 'Kuanzia kinywaji baridi hadi vitafunio vya ufukweni. Vinjari bidhaa zote, chagua vipendwa vyako, nasi tutakuletea.')}</p></div>
            <a href="#delivery-checker" className="shop-delivery-note"><MapPin size={22}/><span><strong>Nungwi & Kendwa</strong><small>{t('Check delivery to your door', 'Angalia ufikishaji mlangoni pako')}</small></span></a>
          </div>
        </section>
        <ShopCategories products={products} selected={category} onSelect={setCategory} loading={loading}/>
      </> : <><section className="hero-wrapper">
        <div className="hero-card-left"><span className="hero-tag">{t('A LITTLE MORE ISLAND TIME', 'MUDA ZAIDI WA KUFURAHIA KISIWA')}</span><h1>{t('Zanzibar drinks.', 'Vinywaji Zanzibar.')}<br/><em>{t('Delivered.', 'Tunakuletea.')}</em></h1><p>{t('Cold drinks, local favourites and beach essentials. Choose what you love. We’ll bring it to your hotel, villa or place by the sea.', 'Vinywaji baridi, vipendwa vya hapa na mahitaji ya ufukweni. Chagua unachopenda. Tutakuletea hotelini, kwenye vila au kando ya bahari.')}</p><div className="hero-cta-group"><Link className="btn-primary-orange" href="/shop">{t('Explore the shop', 'Tembelea duka')}<ArrowRight size={18}/></Link><a className="text-link" href="#delivery-checker">{t('Check your delivery area', 'Angalia eneo lako la ufikishaji')}</a></div><div className="hero-meta-row"><span><MapPin size={17}/> Nungwi & Kendwa</span><span>{t('Your place. Your pace.', 'Mahali pako. Kwa wakati wako.')}</span></div></div>
        <div className="hero-card-right"><SiteImage src="/images/island-vibes-hero-v2.png" alt={t('Chilled island drinks beside the sea in Zanzibar', 'Vinywaji baridi kando ya bahari Zanzibar')} fill sizes="(max-width: 760px) 90vw, 45vw" preload fetchPriority="high"/><div className="hero-caption">{t('A taste of the island.', 'Ladha ya kisiwani.')} <span>ZANZIBAR</span></div></div>
      </section>
      <DeliveryChecker/></>}
      {!shopPage && promotions.length > 0 && <section className="shop-promotions" aria-label={t('Offers')}>{promotions.map(p => <article key={p.id}><span className="eyebrow">{t('An island treat', 'Zawadi ya kisiwani')}</span><h2>{p.title}</h2><p>{p.offer_text}</p><button className="text-link" onClick={() => { sessionStorage.setItem('nungwi-promo', p.code); notify(t('Offer selected for checkout.', 'Ofa imechaguliwa kwa malipo.')) }}>{t('Use offer', 'Tumia ofa')} · {p.code}<ArrowRight size={16}/></button></article>)}</section>}
      <section id="shop" className="catalogue-section" aria-labelledby="catalogue-title"><div className="section-header"><div><span className="eyebrow">{t('THE ISLAND EDIT', 'CHAGUO LA KISIWANI')}</span><h2 id="catalogue-title">{shopPage ? (category === 'All' ? t('All products', 'Bidhaa zote') : t(category)) : t('Find your favourites.', 'Pata vipendwa vyako.')}</h2></div><p>{t('Something refreshing. Something to share. A little of everything you need.', 'Kitu cha kuburudisha. Kitu cha kushiriki. Kila unachohitaji.')}</p></div>
        ...
        {!shopPage && <div className="category-row" aria-label={t('Categories', 'Makundi')}>{categories.map(value => <button key={value} className={`cat-pill ${category === value ? 'active' : ''}`} aria-pressed={category === value} onClick={() => setCategory(value)}>{t(value)}</button>)}</div>}
        <div className="results-bar"><span aria-live="polite">{visible.length} {visible.length === 1 ? t('product', 'bidhaa') : t('products', 'bidhaa')}</span><label><input type="checkbox" checked={availableOnly} onChange={event => setAvailableOnly(event.target.checked)}/>{t('In stock only', 'Zinazopatikana tu')}</label><button className="text-link" onClick={reset}>{t('Clear filters', 'Ondoa vichujio')}</button></div>

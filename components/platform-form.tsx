...
export default function PlatformForm({action,fields,initial={},onSaved,submit='Save'}:{action:string;fields:Field[];initial?:Record<string,any>;onSaved:(value:any)=>void;submit?:string}) {
  ...
  return <form className={styles.form} onSubmit={save}>{fields.map(field=>{
    ...
    return <label key={field.name} className={field.type==='checkbox'?styles.checkbox:''}>{field.type!=='checkbox'&&field.label}{field.type==='select'?<select name={field.name} defaultValue={value} required={field.required!==false}>{field.required===false&&<option value="">None</option>}{field.options?.map(option=><option key={option.value} value={option.value}>{option.label}</option>)}</select>:field.type==='textarea'||field.type==='json'?<textarea name={field.name} defaultValue={value} required={field.required!==false} rows={field.type==='json'?5:3}/>:field.name==='image'?<><input name="image" value={image} onChange={e=>setImage(e.target.value)} placeholder="HTTPS image URL or upload below"/><input type="file" accept="image/png,image/jpeg,image/webp" aria-label="Upload image" onChange={async e=>{const file=e.target.files?.[0];if(!file)return;const form=new FormData();form.set('file',file);setBusy(true);try{const response=await fetch('/api/platform',{method:'POST',body:form});const result=await response.json();if(!response.ok)throw new Error(result.error);setImage(result.data.url)}catch(error){setError((error as Error).message)}finally{setBusy(false)}}}/>{image&&<img className={styles.imagePreview} src={image} alt="Selected image preview"/>}</>:<input name={field.name} type={field.type||'text'} defaultValue={field.type==='checkbox'?undefined:value} defaultChecked={field.type==='checkbox'?Boolean(value):undefined} required={field.type==='checkbox'?false:field.required!==false} min={field.min??(field.type==='number'?0:undefined)} max={field.max} step={field.type==='number'?'1':undefined}/ >}{field.type==='checkbox'&&field.label}{field.help&&<small>{field.help}</small>}</label>
  })}{error&&<p role="alert" className={styles.error}>{error}</p>}<button className={styles.primary} disabled={busy}>{busy?'Saving…':submit}</button></form>
}

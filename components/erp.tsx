...
export default function ERP() {
  ...
  const [newProduct,setNewProduct]=useState<{index:number;query:string}|null>(null)
  ...
  return <>
    {newProduct&&<section className={s.panel}><h2>{t('Add product')}</h2><ProductForm initial={{name:newProduct.query}} onCancel={()=>setNewProduct(null)} onSaved={product=>{setSelectedProducts(v=>({...v,[product.id]:product}));setLines(lines.map((l,i)=>i===newProduct.index?{...l,product_id:product.id,unit_cost:product.cost}:l));setNewProduct(null)}} onUseExisting={product=>{setSelectedProducts(v=>({...v,[product.id]:product}));setLines(lines.map((l,i)=>i===newProduct.index?{...l,product_id:product.id,unit_cost:product.cost}:l));setNewProduct(null)}}/></section>}
    <div hidden={!!newProduct}>
